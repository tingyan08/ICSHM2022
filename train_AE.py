...
def main():
    ...
    with open(f"{args.job_dir}/loss.csv", "w") as f:
        ...
        for epoch in tqdm(range(start_epoch, args.num_epochs)):
            ...
            valid_loss, valid_inputs, valid_outputs = valid(args, valid_dataloader, model)
            utils.plt_reconstruct(args, valid_inputs, valid_outputs, epoch, valid_loss)
...
def train(args, data_loader, model, optimizer, epoch):
    ...
    for inputs, targets in data_loader:
        ...
        # train
        reconsturcted, latent = model(inputs)
        loss = criterion(inputs, reconsturcted)
...
def valid(args, loader_valid, model):
    ...
    return losses.sum, inputs, reconsturcted
